...
public class DHE_RSA_WITH_3DES_CBC_SHA
extends CipherSuite {
    public DHE_RSA_WITH_3DES_CBC_SHA() throws TLSException {
        super(new byte[]{0, 22});
        this.m_ciphersuitename = "TLS_DHE_RSA_WITH_3DES_CBC_SHA";
        this.setKeyExchangeAlgorithm(new DHE_RSA_Key_Exchange());
    }

    protected void calculateKeys(byte[] arrby, boolean bl) {
        this.m_clientwritekey = ByteArrayUtil.copy(arrby, 40, 24);
        this.m_serverwritekey = ByteArrayUtil.copy(arrby, 64, 24);
        this.m_clientwriteIV = ByteArrayUtil.copy(arrby, 88, 8);
        this.m_serverwriteIV = ByteArrayUtil.copy(arrby, 96, 8);
        if (bl) {
