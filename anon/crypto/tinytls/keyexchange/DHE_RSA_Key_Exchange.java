...
extends Key_Exchange {
    ...
    public byte[] generateServerKeyExchange(IMyPrivateKey iMyPrivateKey, byte[] arrby, byte[] arrby2) throws TLSException {
        ...
        byte[] arrby4 = dHPublicKeyParameters.getParameters().getP().toByteArray();
        arrby4 = ByteArrayUtil.conc(ByteArrayUtil.inttobyte(arrby4.length, 2), arrby4);
        byte[] arrby5 = dHPublicKeyParameters.getParameters().getG().toByteArray();
        arrby5 = ByteArrayUtil.conc(ByteArrayUtil.inttobyte(arrby5.length, 2), arrby5);
        byte[] arrby6 = dHPublicKeyParameters.getY().toByteArray();
        arrby6 = ByteArrayUtil.conc(ByteArrayUtil.inttobyte(arrby6.length, 2), arrby6);
        byte[] arrby7 = ByteArrayUtil.conc(arrby4, arrby5, arrby6);
        byte[] arrby8 = ByteArrayUtil.conc(hash.md5(arrby, arrby2, arrby7), hash.sha(arrby, arrby2, arrby7));
        MyRSASignature myRSASignature = new MyRSASignature();
        try {
            myRSASignature.initSign(myRSAPrivateKey);
            arrby3 = myRSASignature.signPlain(arrby8);
        }
        catch (Exception exception) {
            throw new TLSException("cannot encrypt signature", 2, 80);
        }
        arrby7 = ByteArrayUtil.conc(arrby7, ByteArrayUtil.inttobyte(arrby3.length, 2), arrby3);
        return arrby7;
    }
    ...
    public void processServerKeyExchange(byte[] arrby, int n, int n2, byte[] arrby2, byte[] arrby3, JAPCertificate jAPCertificate) throws TLSException {
        ...
        int n3 = 0;
        byte[] arrby4 = ByteArrayUtil.copy(arrby, n3 + n, 2);
        int n4 = (arrby4[0] & 0xFF) << 8 | arrby4[1] & 0xFF;
        byte[] arrby5 = ByteArrayUtil.copy(arrby, (n3 += 2) + n, n4);
        BigInteger bigInteger = new BigInteger(1, arrby5);
        LogHolder.log(7, LogType.MISC, "[SERVER_KEY_EXCHANGE] DH_P = " + bigInteger.toString());
        arrby4 = ByteArrayUtil.copy(arrby, (n3 += n4) + n, 2);
        n4 = (arrby4[0] & 0xFF) << 8 | arrby4[1] & 0xFF;
        arrby5 = ByteArrayUtil.copy(arrby, (n3 += 2) + n, n4);
        BigInteger bigInteger2 = new BigInteger(1, arrby5);
        ...
        byte[] arrby6 = ByteArrayUtil.copy(arrby, 0 + n, n3 += n4);
        byte[] arrby7 = ByteArrayUtil.conc(hash.md5(arrby2, arrby3, arrby6), hash.sha(arrby2, arrby3, arrby6));
        try {
            MyRSASignature myRSASignature = new MyRSASignature();
            myRSASignature.initVerify(jAPCertificate.getPublicKey());
            byte[] arrby8 = ByteArrayUtil.copy(arrby, n3 + 2 + n, n2 - n3 - 2);
            if (!myRSASignature.verifyPlain(arrby7, arrby8)) {
